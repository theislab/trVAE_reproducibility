...
def reconstruct():
    train_path = "../data/haber/train_haber.h5ad"
    data = sc.read(train_path)
    ctrl_key = "Control"
    stim_key = "Hpoly.Day10"
    stim_keys = ['Hpoly.Day3', 'Hpoly.Day10', 'Salmonella']
    all_data = anndata.AnnData()
    print(data.obs["cell_label"].unique().tolist())
    for idx, cell_type in enumerate(data.obs["cell_label"].unique().tolist()):
        pca = PCA(n_components=100)
        train = data[~((data.obs["condition"].isin(stim_keys)) & (data.obs["cell_label"] == cell_type))]
        pca.fit(train.X)
        print(cell_type, end="\t")
        train_real_stimulated = data[data.obs["condition"] == stim_key, :]
        train_real_stimulated = train_real_stimulated[train_real_stimulated.obs["cell_label"] != cell_type]
        train_real_stimulated = scgen.util.balancer(train_real_stimulated, cell_type_key="cell_label")
        train_real_stimulated_PCA = pca.transform(train_real_stimulated.X)

        train_real_cd = data[data.obs["condition"] == ctrl_key, :]
        train_real_cd = scgen.util.balancer(train_real_cd, cell_type_key="cell_label")
        train_real_cd_PCA = pca.transform(train_real_cd.X)

        cell_type_adata = data[data.obs["cell_label"] == cell_type]
        ...
        print(cell_type)
    sc.write("../data/reconstructed/PCA_haber.h5ad", all_data)
...
def train(data_name="pbmc", cell_type="CD4T", p_type="unbiased"):
    ...
    if p_type == "unbiased":
        train_real_cd = scgen.util.balancer(train_real_cd, "cell_label")
    ...
    adata_list = scgen.util.extractor(data, cell_type, {"ctrl": ctrl_key, "stim": stim_key}, cell_type_key=cell_type_key)
