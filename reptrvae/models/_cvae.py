...
class CVAE(Network):
    ...
    def _encoder(self):
        ...
        with tensorflow.variable_scope("encoder", reuse=tensorflow.AUTO_REUSE):
            ...
            h = tensorflow.layers.dense(inputs=xy, units=800, kernel_initializer=self.init_w, use_bias=False)
            h = tensorflow.layers.batch_normalization(h, axis=1, training=self.is_training)
            h = tensorflow.nn.leaky_relu(h)
            h = tensorflow.layers.dense(inputs=h, units=800, kernel_initializer=self.init_w, use_bias=False)
            h = tensorflow.layers.batch_normalization(h, axis=1, training=self.is_training)
            h = tensorflow.nn.leaky_relu(h)
